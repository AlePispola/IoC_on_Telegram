...
# =========================================================
# METRICA 5: Top Offenders (Gruppi + Topic)
# =========================================================
print_header("5. Top 10 Telegram Groups for Confirmed Malware")

# 1. Filtriamo solo le minacce confermate
confirmed_threats = df_combined[df_combined['vt_malicious'] > 0] 

# 2. Raggruppiamo per Nome Gruppo E Topic, poi contiamo
#    (Così manteniamo l'informazione del topic per ogni gruppo)
top_groups = confirmed_threats.groupby(['chat_name', 'topic']).size().reset_index(name='Confirmed Malware Count')

# 3. Ordiniamo dal più grande al più piccolo e prendiamo i primi 10
top_groups = top_groups.sort_values(by='Confirmed Malware Count', ascending=False).head(10)

# 4. Creiamo una nuova colonna "Etichetta" che unisce Nome e Topic
#    Esempio: "NomeGruppo (Darknet)"
top_groups['Label'] = top_groups['chat_name'] + " (" + top_groups['topic'] + ")"

# -- GRAFICO --
plt.figure(figsize=(12, 6)) # Un po' più largo per leggere bene le scritte
sns.barplot(data=top_groups, y='Label', x='Confirmed Malware Count', palette='magma')

plt.title("Top 10 Groups by Confirmed Threat Volume (with Topic)")
plt.xlabel("Number of Malicious IoCs")
plt.ylabel("Telegram Group (Topic)")
plt.tight_layout()
plt.show()
# ==============================================================================
